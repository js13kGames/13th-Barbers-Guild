...
function potion(color: string) {
  return `
    ${move(shape({color: 'rgba(0, 0, 0, 0.5)', width: 66, height: 66, blur: true}), -5, 71)}
    ${move(shape({color, width: 66, height: 66}), 0, 66)}
    ${move(shape({color: 'rgba(0, 0, 0, 0.5)', width: 50, height: 50}), 8, 74)}
  `
}

function shape({ color, width, height, blur }: { color: string; width: number; height: number; blur: boolean }) {
  let filter = '';
  if(blur) {
